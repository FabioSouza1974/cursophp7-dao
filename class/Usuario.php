<?php
    class Usuario {
        ...
        public function login($login, $password){ 
            ...
            if (count($results) > 0) { $this->setData($results[0]); } 
            else { throw new Exception("Login e/ou senha inválidos."); }
        }
        ...
        public function update($login, $password){
            ...
            $sql = new Sql();
            $sql->strquery("UPDATE tb_usuarios SET deslogin = :LOGIN, dessenha = :PASSWORD WHERE idusuario = :ID", array(
                ':LOGIN'=>$this->getDeslogin(),
                ':PASSWORD'=>$this->getDessenha(),
        ...
        public function delete(){
            $sql = new Sql();
            $sql->strquery("DELETE FROM tb_usuarios WHERE idusuario = :ID",array(
                ':ID'=>$this->getIdusuario()
            ));
            //limpando o objeto
            $this->setIdusuario(0);
